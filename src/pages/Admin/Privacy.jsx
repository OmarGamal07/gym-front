...
import { ChangePrivacyHandler } from "../../apis/admin/ChangePrivacy";

const Privacy = () => {
  const [privacy, setUses] = useState();
  const [error, setError] = useState();
  const dispatch = useDispatch();
  const state = useSelector((state) => state.ChangePrivacy);

  const handleForm = () => {
    dispatch(ChangePrivacyHandler({ privacy }));
  };
  const handleStatus = () => {
    if (state.status)
    switch (state.status) {
          ...
          {({ values, errors, handleChange, handleSubmit }) => (
            ...
            >
              <textarea
                ...
                placeholder="تغيير سياسه الخصوصيه"
              >
              </textarea>
              <button
                type="submit"
