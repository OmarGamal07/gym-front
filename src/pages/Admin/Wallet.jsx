...
import { ChangeWalletHandler } from "../../apis/admin/ChangeWallet";

const Wallet = () => {
  const [wallet, setUses] = useState();
  const [error, setError] = useState();
  const dispatch = useDispatch();
  const state = useSelector((state) => state.ChangeWallet);

  const handleForm = () => {
    dispatch(ChangeWalletHandler({ wallet }));
  };
  const handleStatus = () => {
    if (state.status)
    switch (state.status) {
          ...
          {({ values, errors, handleChange, handleSubmit }) => (
            ...
            >
              <textarea
                ...
                placeholder="تغيير وصف محفظه التطبيق "
              >
              </textarea>
              <button
                type="submit"
